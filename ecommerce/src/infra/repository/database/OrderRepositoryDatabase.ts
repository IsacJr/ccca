...
export default class OrderRepositoryDatabase implements OrderRepository {
	...
	async save(order: Order): Promise<void> {
		const [orderData] = await this.connection.query("insert into ccca.order (code, cpf, issue_date, freight, sequence, total, coupon_code, coupon_percentage) values ($1, $2, $3, $4, $5, $6, $7, $8) returning *", [order.code.value, order.cpf.value, order.date, order.shipping.totalValue(), order.sequence, order.calculateTotal(), order.coupon?.code, order.coupon?.percentage]);
	...
	async get(code: string): Promise<Order> {
		...
		order.shipping.total = parseFloat(orderData.freight);
		if (orderData.coupon_code) {
			order.coupon = new Coupon(orderData.coupon_code, orderData.coupon_percentage);
		}
		// alternativa 2
